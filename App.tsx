      ...
      if (data) {
        ...
        const formattedPosts: PostGrowthConfig[] = data.map((v: any) => {
          ...
          return {
            ...
            userDisplayName: v.profiles?.display_name || v.profiles?.username || 'Usuario ANIX',
            caption: v.caption || "",
  ...
  const checkDailyUploadReminder = useCallback((currentPosts: VideoPost[]) => {
    ...
    if (!hasUploadedToday) {
      ...
      if ('Notification' in window && Notification.permission === 'granted') {
        new window.Notification("¡Hora de ANIX!", {
  ...
  const handleUpload = async (videoFile: File, caption: string) => {
    ...
    try {
      ...
      const profilePromise = supabase.from('profiles').upsert({
        ...
        display_name: user.displayName || 'Usuario ANIX',
    ...
    <div className="relative h-[100dvh] w-full bg-black overflow-hidden select-none">
      ...
      {showOnboarding && activeTab === 'home' && (
        <div className="fixed inset-0 z-[200] bg-black flex flex-col items-center justify-center p-12 text-center animate-fade-in">
          ...
          <h1 className="text-4xl font-black mb-4 tracking-tighter uppercase">ANIX</h1>
      ...
      {isLoading && (
        <div className="fixed inset-0 z-[150] bg-black/95 flex flex-col items-center justify-center p-6 animate-fade-in">
          ...
          <p className="text-white font-bold tracking-widest uppercase text-xs">
            {uploadProgress !== null ? `Subiendo video: ${uploadProgress}%` : 'Cargando ANIX...'}
          </p>
          {uploadProgress !== null && (
